        ...
        for row in now_a:
            ...
            for v in row:
                if v in cnt:
                    ...
                    cnt[v] = 1
            new_line = []
            for key in sorted(cnt, key=lambda x: cnt[x]):
                new_line.extend([key, cnt[key]])
            new_a.append(new_line)
            max_length = max(max_length, len(new_line))

        max_length = max_length if max_length <= 100 else 100
        for i, line in enumerate(new_a):
            new_a[i] = line[:max_length] + [0]*(max_length-len(line))

        return new_a, len(new_a), max_length
